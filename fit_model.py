...
def train(df, params):
    ...
    for t in range(99,-1,-1):
        if t % 10 != 0 and t not in (99,98,97,96,95,94,93,92,91,90,0):
            ...
            continue
        a = np.percentile(y_prob,t)
        result['predict'] = result['prob'].apply(lambda x: 1 if x >= a else 0)
        count = result[result['predict'] == 1].shape[0]
        count_true = result[(result['predict'] == 1 ) & (result['label'] == 1)].shape[0]
        recall = count_true/result[result['label']==1].shape[0]
        accuracy = count_true/count
        up_rate = accuracy/(result[result['label']==1].shape[0]/result.shape[0])
